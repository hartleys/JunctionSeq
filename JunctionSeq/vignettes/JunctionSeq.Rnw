...
v0.6.10 %REPLACE_THIS_LINE_WITH_VERSION_NUMBER
}
...
JunctionSeq requires R v3.2.2 as well as a number of R packages. The easiest way to install it is to use the automated installer. Note that to install JunctionSeq on windows, you must have \href{https://cran.r-project.org/bin/windows/Rtools/}{Rtools}.
