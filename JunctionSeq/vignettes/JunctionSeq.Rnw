...
JunctionSeq requires R v3.2.2 as well as a number of R packages. See the [JunctionSeq website](http://hartleys.github.io/JunctionSeq/) for more detailed and updated information on JunctionSeq installation. In general, the easiest way to install it is to use the automated installer:
