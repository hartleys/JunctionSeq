...
\emph{Software:} JunctionSeq requires the QoRTs software package to produce the flattened annotation files and splice-junction count files necessary for analysis. The QoRTs software package requires R version 3.0.2 or higher, as well as java 6 or higher. JunctionSeq itself requires a number of R packages, which can be installed using the following R commands:
...
\emph{Hardware:} Both the JunctionSeq and QoRTs software packages will generally require at least 4 gigabytes of RAM to run. In general at least 8gb is recommended if available.
...
\emph{Dataset:} JunctionSeq requires aligned RNA-Seq data. Data can be paired-end or single-end, unstranded or stranded. It is strongly recommended, but not explicitly required, that the SAM/BAM files be sorted either by name or position (it does not matter which).
...
For example, to run QoRTs on the first read-group of sample \verb|S_D1| from the example dataset:
...
If Quality Control is being done seperately by other software packages or collaborators, the JunctionSeq counts can be generated alone by setting the \verb|--runFunctions| option:
\begin{framed} \begin{verbatim}
java -jar /path/to/jarfile/QoRTs.jar QC \
 --stranded \
...
For more information about the quality control metrics provided by QoRTs, and how to visualize, organize, and view them, see the QoRTs github page and documentation, available online \href{http://github.com/hartleys/QoRTs}{here}.
...
For each selected gene, this function will produce 8 plots, two each of four plot types:
  \begin{itemize}
    \item \emph{geneID-expr.png}: Estimates of average coverage depth for each biological condition. See Section \ref{sec:subplot1}.
    \item \emph{geneID-rExpr.png}: Relative expression estimates for each biological condition (excluding gene-level differential effects). See Section \ref{sec:subplot2}.
    \item \emph{geneID-rawCounts.png}: Raw read counts for each sample, colored by biological condition. See Section \ref{sec:subplot4}.
  ...
  \end{itemize}
Two versions of each plot are generated: one in which the full annotated transcript set is plotted beneath the expression graph, and one where only the simplified composite annotation is displayed.
...
Figure \ref{../test5-ENSRNOG00000009281-expr-withTx-fig} displays the same information found in Figure \ref{../test5-ENSRNOG00000009281-expr-fig}, except with all known transcripts plotted beneath the main plot.

\myincfigRaw{../test5-ENSRNOG00000009281-expr-withTx}{0.65\textwidth}{Splice junction coverage by biological condition, with annotated transcripts displayed. This plot is identical to the previous, except all annotated transcripts are displayed below the standard plot.}
...
\myincfigRaw{../test5-ENSRNOG00000009281-rawCounts}{0.85\textwidth}{Raw counts for each sample}

Figure \ref{../test5-ENSRNOG00000009281-rawCounts-fig} displays the raw (un-normalized) coverage counts for each sample over each splice junction. This is equal to the number of reads (or read-pairs, for paired-end data) that bridge each junction. Note that these counts are not normalized, and are generally not directly comparable.
...
Splice Junction Tracks are generated automatically by the \verb|writeCompleteResults| function, assuming the \verb|write.bedTracks| parameter is TRUE (the default). By default, three sets of junction tracks are generated: 

  \begin{itemize}
    \item \emph{allGenes.coverage.bed.gz}: A track that lists the mean normalized read (or read-pair) coverage for each splice junction and for each biological condition. These are not equal to the actual mean normalized counts, rather these are derived from the parameter estimates.
    \item \emph{sigGenes.coverage.bed.gz}: This track is identical to the previous one except that only junctions belonging to genes that contain at least one statistically significant junction are included. All other genes are dropped. This makes the file much smaller and more portable while retaining the most useful information.
    \item \emph{sigGenes.pvalues.bed.gz}: This track lists all statistically-significant splice junction loci, with the adjusted p-value listed in parentheses.
  \end{itemize}
...
Individual-sample or individual-replicate junction coverage tracks can also be generated via QoRTs. See the QoRTs documentation available \href{https://github.com/hartleys/QoRTs}{online}.
