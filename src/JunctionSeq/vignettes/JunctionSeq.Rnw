...
have higher counts than the individual splice junctions (particularly for large exons) and as such tests for differential exon usage will often have higher power under ideal conditions. However, certain combinations of isoform
...
JunctionSeq requires R v3.1.1 as well as a number of R packages. The easiest way to install it is to use the automated installer. Note that to install JunctionSeq on windows, you must have \href{https://cran.r-project.org/bin/windows/Rtools/}{Rtools}.
...
\emph{Generating splice junction counts:} The easiest way to generate the counts for JunctionSeq is via the QoRTs software package. The QoRTs software package requires R version 3.0.2 or higher, as well as (64-bit) java 6 or higher. QoRTs can be found online \href{http://hartleys.github.io/QoRTs/index.html}{here}.

\emph{Hardware:} Both the JunctionSeq and QoRTs software packages will generally require at least 2-4 gigabytes of RAM to run. In general at least 4gb is recommended when available.
...
To allow users to test JunctionSeq and experiment with its functionality, an example dataset is available online \href{http://hartleys.github.io/JunctionSeq/install/JctSeqExData2_LATEST.tar.gz}{here}. It can be installed with the command:
<<installExData, eval = FALSE>>=
install.packages("http://hartleys.github.io/JunctionSeq/install/JctSeqExData2_LATEST.tar.gz", repos=FALSE, type = "source");
@
...
Two additional sets of minor optional plots which may be useful to some users can be added using the command:
<<makeAllPlots2>>=
buildAllPlots(jscs=jscs,
              outfile.prefix = "./plots/",
              use.plotting.device = "png",
...
Figure \ref{../plots/rExpr/ENSRNOG00000009281-rExpr-fig} displays the "relative" coverage for each splice junction or exonic region, relative to gene-wide expression. Note that, by default, this plot is NOT generated by \verb|buildAllPlots|.  Generation of these plots must be turned on by adding the parameter: "\verb|rExpr.plot=TRUE|".

JunctionSeq is designed to detect differential expression even in the presence of gene-wide, multi-transcript differential expression. However it can be difficult to visually assess differential usage on differentially exprssed genes. This plot displays the coverage relative to the gene-wide expression. These estimates are derived from the GLM parameter estimates (via linear contrasts).
...
\myincfigRaw{../plots/rawCounts/ENSRNOG00000009281-rawCounts}{0.85\textwidth}{\footnotesize Raw counts for each sample}
...
All of these optional alternatives can be selected using the "method" parameters passed to \verb|runJunctionSeqAnalyses| and its sub-functions (see Section \ref{sec:DJUtestAdv}). These optional parameters are:
...
This will precisely reproduce the exact analyses that would be generated by the following DEXSeq commands in DEXSeq v1.12.1:
<<DEXRun, eval=FALSE>>=
library("DEXSeq");
