...
# JunctionSeq modification: Due to the model matrix absorbing ,
#    The DEXSeq version of this function ALWAYS returns NA.
#    as it considers each "sample" as its own bin.
# Clearly this isn't the desired behavior.
recordMaxCooks <- function(design, colData, modelMatrix, cooks, numRow) {
    ##Old version:
    samplesForCooks <- nOrMoreInCell(modelMatrix, n=3)
    ##new version:
    ##samplesForCooks <- rep(TRUE, nrow(modelMatrix));
    p <- ncol(modelMatrix)
    m <- nrow(modelMatrix)
